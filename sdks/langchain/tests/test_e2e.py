...
class TestE2EClient:
    ...
    @pytest.mark.asyncio
    async def test_run_tool1(self, toolbox, auth_token2):
    ...
    @pytest.mark.asyncio
    async def test_run_tool2(self, toolbox, auth_token2):
        tool = await toolbox.load_tool(
    ...
    @pytest.mark.asyncio
    async def test_run_tool_auth_param_no_auth(self, toolbox, auth_token1):
        tool = await toolbox.load_tool("get-row-by-email-auth")
        with pytest.raises(PermissionError, match="Login required"):
            await tool.arun({})

    @pytest.mark.asyncio
    async def test_run_tool_auth_param(self, toolbox, auth_token1):
        tool = await toolbox.load_tool(
